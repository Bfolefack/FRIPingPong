...
class SpawnModel:
    def __init__(self):
        ...
        self.ballDropHeightThreshold = 0.4
        self.ballBelowThreshold = False

        # Setting up the subscriber & ROS
        rospy.init_node('ball_model_spawner')
        rospy.Subscriber("%s/model_states"%(self.gazebo_namespace), ModelStates, self.checkForModel, queue_size=2, buff_size=2**24)
        rospy.on_shutdown(self.deleteServiceFinal)
    ...
    def ballSpawner(self, vel):
        if self.readyToSpawn and not self.ballExists:
            ...
            # Get filepath of sdf of ball
            ball_sdf_file_path = pong_gazebo_pkg_file_path + '/models/ball/ball.sdf'

            # Copies the contents of the ball sdf file into ball_model_xml
            with open(ball_sdf_file_path, 'r') as ball_sdf_file:
                ball_model_xml = ball_sdf_file.read()

            initial_ball_pose = Pose()
            initial_ball_pose.position.x = 2#3.4932 #TODO
            ...
            # The arguments for gazebo_interface.spawn_sdf_model which aren't define in this ballSpawner class are instance variables defined in constructor
            success = gazebo_interface.spawn_sdf_model_client(self.model_name, ball_model_xml, self.robot_namespace, initial_ball_pose, self.reference_frame, self.gazebo_namespace)
    ...
    '''
    Sets the velocity of the ball sdf xml by substituting
    ...
    self:
    ball_model_xml: xml
        The sdf xml for the ball to be spawned
    ...
    ------------------ 
    ball_model_xml: xml
        The UPDATED sdf xml for the ball to be spawned 
    '''
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            ...
            # rospy.sleep(1)
        # rospy.spin() is not necessary b/c while not rospy.is_shutdown() takes care of it for us
